...
class Main(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        label = ttk.Label(
            ...
            font=font.Font(family="Edwardian Script ITC", size=20),
        )
        label.grid(row=0, column=40, padx=10, pady=10)
        button1 = ttk.Button(
            self, text="Bisection", command=lambda: controller.showFrame("BisectionGui")
        )
        button1.grid(row=1, column=1, padx=10, pady=10)

        button2 = ttk.Button(
            self, text="Fixpoint", command=lambda: controller.showFrame("FixpointGui")
        )
